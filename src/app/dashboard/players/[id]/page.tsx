    ...
    <div className="space-y-6 max-w-6xl">
      <div className="flex flex-col md:flex-row md:items-start gap-5 md:justify-between">
        <div className="flex gap-4 items-center">
          <div className="w-20 h-20 rounded-xl bg-slate-100 flex items-center justify-center text-4xl overflow-hidden">
            {player.photo_url ? (
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <div className="card p-4"><div className="label">Matches</div><div className="text-2xl font-bold text-brand-700">{performances.length}</div></div>
        <div className="card p-4"><div className="label">Goals</div><div className="text-2xl font-bold text-brand-700">{totalGoals}</div></div>
        <div className="card p-4"><div className="label">Assists</div><div className="text-2xl font-bold text-brand-700">{totalAssists}</div></div>
        <div className="card p-4"><div className="label">Avg rating</div><div className="text-2xl font-bold text-brand-700">{avgRating}</div></div>
      </div>

      <section className="card p-5">
        <h2 className="font-semibold text-slate-900 mb-3">Profile details</h2>
        <dl className="grid sm:grid-cols-2 md:grid-cols-3 gap-y-3 gap-x-6 text-sm">
        ...
        {player.notes && <p className="text-sm text-slate-700 mt-4 leading-relaxed whitespace-pre-line">{player.notes}</p>}
      </section>

      <section className="card p-5">
        <h2 className="font-semibold text-slate-900 mb-3">Recent matches</h2>
            ...
            <table className="w-full text-sm">
              <thead><tr className="bg-slate-50 border-y border-slate-200">
                {['Date','Opponent','Min','G','A','Passes','Shots','Rating'].map((h) => (
                  <th key={h} className="px-5 py-2 text-left text-xs font-semibold uppercase text-slate-600">{h}</th>
                ))}
              </tr></thead>
              <tbody className="divide-y divide-slate-100">
                {performances.map((p) => (
                  <tr key={p.id}>
                    <td className="px-5 py-2 text-slate-700">{formatDate(p.match_date)}</td>
                    <td className="px-5 py-2 text-slate-700">{p.opponent}</td>
                    <td className="px-5 py-2">{p.minutes_played}</td>
                    <td className="px-5 py-2 font-medium">{p.goals}</td>
                    <td className="px-5 py-2">{p.assists}</td>
                    <td className="px-5 py-2">{p.passes_completed}/{p.passes_attempted}</td>
                    <td className="px-5 py-2">{p.shots_on_target}/{p.shots}</td>
                    <td className="px-5 py-2 font-medium">{p.rating ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="card p-5">
        <h2 className="font-semibold text-slate-900 mb-3">Injury history</h2>
        {injuries.length === 0 ? (
          ...
          <ul className="divide-y divide-slate-100">
            {injuries.map((inj) => (
              <li key={inj.id} className="py-3 flex items-start justify-between gap-4">
                <div>
                  <div className="font-medium text-slate-800">{inj.injury_type} — {inj.body_part}</div>
      ...
      {latestEval && (
        <section className="card p-5">
          <h2 className="font-semibold text-slate-900 mb-3">Latest evaluation ({formatDate(latestEval.evaluation_date)})</h2>
...
function Pair({ label, value }: { label: string; value: React.ReactNode }) {
  return <div><div className="label">{label}</div><div className="text-slate-800">{value}</div></div>;
}

function EvalBlock({ title, items }: { title: string; items: [string, number | null][] }) {
  return (
    <div>
      <div className="font-semibold text-brand-700 mb-2">{title}</div>
      <ul className="space-y-1">
        {items.map(([k, v]) => (
          <li key={k} className="flex justify-between text-slate-700">
            <span>{k}</span><span className="font-medium">{v ?? '—'}/10</span>
          </li>
        ))}
      </ul>
    </div>
