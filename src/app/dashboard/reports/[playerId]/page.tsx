...
export default function PlayerReportPage({ params }: { params: { playerId: string } }) {
  ...
  async function saveEvaluation() {
    setSaving(true);
    const res = await fetch('/api/performances', { method: 'OPTIONS' }); // no-op, reuse session
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Report · {player.first_name} {player.last_name}</h1>
          <p className="text-sm text-slate-500">Fill the evaluation below and download the full PDF report.</p>
        </div>
      ...
      <div className="card p-5 space-y-4">
        <div className="flex flex-col gap-1">
          ...
          <textarea className="input-base min-h-[80px]" value={evaluation.strengths ?? ''} onChange={(e) => setEvaluation({ ...evaluation, strengths: e.target.value })} />
        </div>
        <div className="flex flex-col gap-1">
          <label className="label">Areas to improve</label>
          <textarea className="input-base min-h-[80px]" value={evaluation.areas_to_improve ?? ''} onChange={(e) => setEvaluation({ ...evaluation, areas_to_improve: e.target.value })} />
        </div>
        <div className="flex flex-col gap-1">
          <label className="label">General notes</label>
          <textarea className="input-base min-h-[80px]" value={evaluation.general_notes ?? ''} onChange={(e) => setEvaluation({ ...evaluation, general_notes: e.target.value })} />
        </div>
      </div>
    </div>
  );
