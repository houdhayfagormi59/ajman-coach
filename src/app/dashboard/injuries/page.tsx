  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Injuries</h1>
          <p className="text-sm text-slate-500">Track injuries and recovery</p>
        </div>
      ...
      <div className="flex gap-2">
        {(['active', 'recovered', 'all'] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium ${filter === f ? 'bg-brand-500 text-white' : 'bg-white border border-slate-200 text-slate-600'}`}
          >
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((inj) => (
                  <tr key={inj.id}>
                    ...
                    <td className="px-5 py-3 text-slate-700">{inj.severity}</td>
                    ...
                    <td className="px-5 py-3 flex gap-1">
                      {inj.status === 'active' && (
                        <button onClick={() => markRecovered(inj.id)} className="text-green-700 hover:bg-green-50 p-1 rounded" title="Mark recovered">
                          <CheckCircle2 size={16} />
                        </button>
                      )}
                      <button onClick={() => remove(inj.id)} className="text-red-600 hover:bg-red-50 px-2 py-1 rounded text-xs">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
