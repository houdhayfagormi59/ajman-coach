...
export default function PerformanceForm({ players, onDone }: { players: Pick<Player, 'id' | 'first_name' | 'last_name'>[]; onDone?: () => void }) {
  ...
  const upd = (k: keyof typeof data) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setData({ ...data, [k]: ['minutes_played','goals','assists','passes_completed','passes_attempted','tackles','shots','shots_on_target'].includes(k as string) ? Number(e.target.value) : k === 'rating' ? Number(e.target.value) : e.target.value });
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Input label="Opponent" required value={data.opponent} onChange={upd('opponent') as any} />
